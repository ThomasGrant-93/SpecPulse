...
export default function SettingsPage() {
    ...
    const updateMutation = useMutation({
        mutationFn: async ({ category, key, value }: { category: string; key: string; value: SettingValue }) => {
        ...
        <div>
            <div className="sm:flex sm:items-center">
                <div className="sm:flex-auto">
                    ...
                    <p className="mt-2 text-sm text-gray-700">Configure application settings, scheduler, notifications, and security</p>
                </div>
            </div>
                                ...
                                <div>
                                    {selectedCategoryData.settings.map((setting) => (
                                        <SettingField
                                            ...
                                            onChange={(newValue) =>
                                                setEditingSetting({ key: setting.key, value: newValue })
                                            }
                                            onSave={() => {
                                                if (editingSetting) {
                                                    const valueToSave =
                                                        editingSetting.key === setting.key
                                                            ? editingSetting.value
                                                            : setting.value;
                                                    handleSave(setting.category, setting.key, valueToSave);
                                                } else {
                                                    handleSave(setting.category, setting.key, setting.value);
                                                }
                                            }}
